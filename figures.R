...
plot.frame$model[plot.frame$model=="Paramedics Masked Only"]<-"B. First Responders with Respirators"
...
A<-ggplot(plot.frame.temp[plot.frame.temp$time<=5000,])+geom_line(aes(x=time*0.001,y=means,group=interaction(state,model),color=state,linetype=state),size=1.1)+
  ...
  scale_y_continuous(name="Average Number of Viral Particles",trans="log10")+
...
frame.temp.2$fraction[frame.temp.2$time==12500 & frame.temp.2$type=="Respiratory Tract" & frame.temp.2$model=="Patient Respirator Only"]
frame.temp.2$fraction[frame.temp.2$time==12500 & frame.temp.2$type=="Mucous Membrane" & frame.temp.2$model=="Patient Respirator Only"]

frame.temp.2$fraction[frame.temp.2$time==12500 & frame.temp.2$type=="Respiratory Tract" & frame.temp.2$model=="First Responder Respirators Only"]
frame.temp.2$fraction[frame.temp.2$time==12500 & frame.temp.2$type=="Mucous Membrane" & frame.temp.2$model=="First Responder Respirators Only"]

frame.temp.2$fraction[frame.temp.2$time==12500 & frame.temp.2$type=="Respiratory Tract" & frame.temp.2$model=="Patient and First Responder Respirator"]
frame.temp.2$fraction[frame.temp.2$time==12500 & frame.temp.2$type=="Mucous Membrane" & frame.temp.2$model=="Patient and First Responder Respirator"]
...
air.conc.sum<-rep(NA,4000)

for (i in 1:4000){
  if(i<=1000){
    scenario<-"C. Patient with Mask"
  }else if (i>1000 & i<=2000){
    scenario<-"A. No One with Respirators or Masks"
  }else if (i>2000 & i<=3000){
    scenario<-"B. First Responders with Respirators"
  }else{
    scenario<-"D. Patient and First Responders with Mask or Respirators"
  }
  ...
  air.conc.sum[i]<-tempframe.airborne$state1[finaltime]
...
infect.frame$scenario<-factor(infect.frame$scenario,levels=c("A. No One with Respirators or Masks", "B. First Responders with Respirators","C. Patient with Mask",
                                "D. Patient and First Responders with Mask or Respirators"))
windows()
...
baseline<-mean(infect.frame$infect[infect.frame$model=="Scenario 1" & infect.frame$scenario=="A. No One with Respirators or Masks" ])

summary(infect.frame$infect[infect.frame$model=="Scenario 1" & infect.frame$scenario=="A. No One with Respirators or Masks" ])
sd(infect.frame$infect[infect.frame$model=="Scenario 1" & infect.frame$scenario=="A. No One with Respirators or Masks" ])

#First responder w/ respirator
int.1<-mean(infect.frame$infect[infect.frame$model=="Scenario 1" & infect.frame$scenario=="B. First Responders with Respirators" ])

(baseline-int.1)/baseline*100

#Patient w/ respirator
int.2<-mean(infect.frame$infect[infect.frame$model=="Scenario 1" & infect.frame$scenario=="C. Patient with Mask" ])

(baseline-int.2)/baseline*100

#Both w/ respirator
int.3<-mean(infect.frame$infect[infect.frame$model=="Scenario 1" & infect.frame$scenario=="D. Patient and First Responders with Mask or Respirators" ])

summary(infect.frame$infect[infect.frame$model=="Scenario 1" & infect.frame$scenario=="D. Patient and First Responders with Mask or Respirators" ])
sd(infect.frame$infect[infect.frame$model=="Scenario 1" & infect.frame$scenario=="D. Patient and First Responders with Mask or Respirators" ])
...
#% changes in infection risk 
baseline<-mean(infect.frame$infect[infect.frame$model=="Scenario 2" & infect.frame$scenario=="A. No One with Respirators or Masks" ])

summary(infect.frame$infect[infect.frame$model=="Scenario 2" & infect.frame$scenario=="A. No One with Respirators or Masks" ])
sd(infect.frame$infect[infect.frame$model=="Scenario 2" & infect.frame$scenario=="A. No One with Respirators or Masks" ])

#First responder w/ respirator
int.1<-mean(infect.frame$infect[infect.frame$model=="Scenario 2" & infect.frame$scenario=="B. First Responders with Respirators" ])

(baseline-int.1)/baseline*100

#Patient w/ respirator
int.2<-mean(infect.frame$infect[infect.frame$model=="Scenario 2" & infect.frame$scenario=="C. Patient with Mask" ])
